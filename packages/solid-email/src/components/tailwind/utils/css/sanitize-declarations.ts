...
function oklchToRgb(oklch: { l: number; c: number; h: number }) {
  ...
  const [labToLmsL0, labToLmsL1] = LAB_TO_LMS.l;
  const [labToLmsM0, labToLmsM1] = LAB_TO_LMS.m;
  const [labToLmsS0, labToLmsS1] = LAB_TO_LMS.s;
  const l = (oklab.l + labToLmsL0 * oklab.a + labToLmsL1 * oklab.b) ** 3;
  const m = (oklab.l + labToLmsM0 * oklab.a + labToLmsM1 * oklab.b) ** 3;
  const s = (oklab.l + labToLmsS0 * oklab.a + labToLmsS1 * oklab.b) ** 3;

  const [lsmToRgbR0, lsmToRgbR1, lsmToRgbR2] = LSM_TO_RGB.r;
  const [lsmToRgbG0, lsmToRgbG1, lsmToRgbG2] = LSM_TO_RGB.g;
  const [lsmToRgbB0, lsmToRgbB1, lsmToRgbB2] = LSM_TO_RGB.b;
  const r = 255 * lrgbToRgb(lsmToRgbR0 * l + lsmToRgbR1 * m + lsmToRgbR2 * s);
  const g = 255 * lrgbToRgb(lsmToRgbG0 * l + lsmToRgbG1 * m + lsmToRgbG2 * s);
  const b = 255 * lrgbToRgb(lsmToRgbB0 * l + lsmToRgbB1 * m + lsmToRgbB2 * s);
...
function separteShorthandDeclaration(
  ...
  const [startValue, endValueNode] = values;
  if (startValue && endValueNode) {
    endValue = {
      type: 'Value',
      children: new List<CssNode>().fromArray([endValueNode]),
    };
    shorthandToReplace.value = {
      type: 'Value',
      children: new List<CssNode>().fromArray([startValue]),
    };
  }
